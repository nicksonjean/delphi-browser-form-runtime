unit BrowserInterface;

interface

uses
  Vcl.Forms, BrowserTypes, BrowserGenericInterface;

type
  IBrowser = interface(IBrowserGeneric)
  ['{F8CB52A5-3F59-4E3E-B2B7-98478AB77A5F}']
    // Fluent methods
    function SetWidth(const AWidth: Integer): IBrowser;
    function SetHeight(const AHeight: Integer): IBrowser;
    function SetCaption(const ACaption: String; APosition: TPositionCaption = TPositionCaption.Before): IBrowser;
    function SetActionButtons(const AButtons: TBorderIcons): IBrowser;
    function SetResizable(const AResize: Boolean): IBrowser;
    function SetMovable(const AMove: Boolean): IBrowser;
    procedure Show(const AType: TOpenType = TOpenType.Normal);
    procedure ShowModal(const AType: TOpenType = TOpenType.Modal);
  end;

implementation

end.
